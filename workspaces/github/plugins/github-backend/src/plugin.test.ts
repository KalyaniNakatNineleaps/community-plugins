...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [githubPlugin],
    });

    await request(server).get('/api/github/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/github/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/github/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/github/todos/${createdTodoItem.id}`)
